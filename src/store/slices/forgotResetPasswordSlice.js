...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://websitebackend-production-7b1e.up.railway.app/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        ` https://websitebackend-production-7b1e.up.railway.app/api/v1/user/password/reset/${token}`,
